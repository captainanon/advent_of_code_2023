...
cl_inputs = defaultdict(lambda: -1)
flag = 0
    ...
    while queue != []:
        ...
        if node == 'broadcaster':
            ...
            for input in conjunctions[node]:
                state *= input[1]
                if node == 'cl' and input[1] == 1 and cl_inputs[input[0]] == -1:
                    cl_inputs[input[0]] = i
                    if len(list(filter(lambda x: x == -1, cl_inputs.values()))) == 0:
                        print(list(filter(lambda x: x == -1, cl_inputs.values())))
                        flag = 1
            output = 1 if state == 0 else 0
        for neighnbor in graph[node]:
            queue.append((output, neighnbor, node))
print(math.lcm(*(cl_inputs.values()))) # 250628960065793

...
def solve(input):
    with open(input) as file:
        ...
        for idx, lst in enumerate(hands):
            ...
            hands[idx][0] = hand
            dummy_hand = copy.copy(hand)
            jokers = dummy_hand.count('n')
            dummy_hand = dummy_hand.replace('n', '')
            vals = dict(Counter(dummy_hand))
            ...
            if jokers == 5:
                ...
                if list(counts.keys())[0] == 3:
                    hands[idx].insert(0, 7)
                elif list(counts.keys())[0] == 2:
                    hands[idx].insert(0, 6)
                else:
                    hands[idx].insert(0, 4)
            elif jokers == 1:
                if list(counts.keys())[0] == 4:
                    hands[idx].insert(0, 7)
                elif list(counts.keys())[0] == 3:
                    hands[idx].insert(0, 6)
...
def map_cards(hand):
    original_cards = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J']
